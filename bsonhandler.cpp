...
bsoncxx::document::value BsonHandler::newEntry(QRegularExpressionMatch match)
{
    ...
    int64_t mseconds =
        QDateTime::fromString(match.captured("entryDate") + match.captured("entryTime"), "dd/MM/yyhh:mm:ss")
            .addYears(100)
            .toMSecsSinceEpoch();

    bsoncxx::document::value document = make_document(
        kvp("$setOnInsert",
            make_document(kvp("soNumber", b_int32{match.captured("soNumber").toInt()}), kvp("open", b_bool{true}),
                          kvp("entry", b_date{std::chrono::milliseconds{mseconds}}), kvp("finished", b_null{}))),
        kvp("$addToSet", make_document(kvp(
                             "jobs", make_document(kvp("jobName", b_utf8{match.captured("jobName").toStdString()}),
                                                   kvp("jobNumber", b_int32{match.captured("jobNumber").toInt()}),
                                                   kvp("jobStatus", b_utf8{"queue"}),
                                                   kvp("machine", b_utf8{match.captured("machine").toStdString()}))))));
...
{
    bsoncxx::document::value filter =
        make_document(kvp("soNumber", b_int32{match.captured("soNumber").toInt()}),
                      kvp("jobs.jobNumber", b_int32{match.captured("jobNumber").toInt()}));
    ...
    bsoncxx::document::value filter = make_document(
        ...
        kvp("jobs",
            make_document(kvp(
                ...
                make_document(
                    kvp("jobNumber", b_int32{match.captured("jobNumber").toInt()}),
                    kvp("steps", make_document(kvp("$elemMatch", make_document(kvp("program", "PWEWTO2"),
...
{
    int64_t mseconds =
        QDateTime::fromString(match.captured("executionDate") + match.captured("executionTime"), "dd/MM/yyhh:mm:ss")
            .addYears(100)
            .toMSecsSinceEpoch();

    bsoncxx::document::value document = make_document(
        kvp("$set", make_document(kvp("jobs.$.started", b_date{std::chrono::milliseconds{mseconds}}),
                                  kvp("jobs.$.jobStatus", b_utf8{"running"}), kvp("jobs.$.finished", b_null{}))));
...
bsoncxx::document::value BsonHandler::processing(QRegularExpressionMatch match)
{
    ...
    bsoncxx::document::value document = make_document(
        kvp("$addToSet",
            make_document(kvp("jobs.$.steps",
                              make_document(kvp("stepName", b_utf8{match.captured("stepName").toStdString()}),
...
bsoncxx::document::value BsonHandler::ended(QRegularExpressionMatch match)
{
    ...
    bsoncxx::document::value document = make_document(
        kvp("$set", make_document(kvp("jobs.$.jobStatus", b_utf8{"finished"}),
...
bsoncxx::document::value BsonHandler::check()
{
    bsoncxx::document::value document =
        make_document(kvp("$set", make_document(kvp("jobs.$.jobStatus", b_utf8{"error"}))));
...
bsoncxx::document::value BsonHandler::closeOS(QRegularExpressionMatch match)
{
    ...
    bsoncxx::document::value document =
        make_document(kvp("$set", make_document(kvp("open", b_bool{false}),
                                                kvp("finished", b_date{std::chrono::milliseconds{mseconds}}))));
